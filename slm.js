    ...
    var fl={
		...
		splitV:function()
        {
            ...
            if (!t.ok)
            {
                ...
                var drag=-1;
			    spl.bind("mousedown touchstart",function(e){ drag=e.pageY-t.sash;});
				self.bind("mouseup touchend",function(e){ drag=-1;window.event.returnValue = true;});
			    self.bind("mousemove touchmove",function(e){if(drag>=0){t.sash=e.pageY-drag;setT(self,t);self.slm();window.event.returnValue = false;}});
                t.ok=1;
                setT(self,t);
            }
            //adattamento children	
		...
		splitH:function()
        {
            ...
            if (!t.ok)
            {
                ...
                var drag=-1;
			    spl.bind("mousedown touchstart",function(e){ t=getT(self);drag=e.pageX-t.sash;});
				self.bind("mouseup touchend",function(e){ drag=-1;});
			    self.bind("mousemove touchmove",function(e){if(drag>=0){t.sash=e.pageX-drag;setT(self,t);self.slm();window.event.returnValue = false;}});
                t.ok=1;
                setT(self,t);
            }			
            var p0=$(c[0]);
